import hashlib
import os
from binascii import hexlify

class D_H(object):
    def __init__(self, generator=2, keylength=620):
        # Generate the public and private keys.

        self.generator = generator
        self.keyLength = keylength

        self.prime = self.getPrime()
        self.privint = self.genPrivint(self.keyLength)
        self.pubKey = self.genPubKey()

    def getPrime(self):
        # function to fetch the prime.
        prime = 
        return prime

    def genRandom(self, bits):
        bytes = bits // 8 + 1
        rand = int(hexlify(os.urandom(bytes)), 16)
        return rand

    def genPrivint(self, bits):
        # Generate our secret int that is used for the first computation to generate the first key.
        return self.genRandom(bits)

    def genPubKey(self):
        # Compute g^(secret int) % prime
        return pow(self.generator, self.privint, self.prime)

    def genS(self, privint, someKey):
        # compute our final share secret key. somekey = senders "public" computed key.
        return pow(someKey, privint, self.prime)

    def genKey(self, someKey):
        sharedSecret = self.genS(self.privint, someKey)
        _sharedSecretBytes = str(sharedSecret)

        s = hashlib.sha256()
        s.update(_sharedSecretBytes.encode())
        self.key = s.digest()

    def getKey(self):
        return self.key